...
inputdirectories <- c("/Users/jess/RajLab Dropbox/Jess Li/Shared_JessL/paper/extractedData/dexmemfish/rep1", 
                      "/Users/jess/RajLab Dropbox/Jess Li/Shared_JessL/paper/extractedData/dexmemfish/rep2", 
                      "/Users/jess/RajLab Dropbox/Jess Li/Shared_JessL/paper/extractedData/dexmemfish/rep3/primary",
                      "/Users/jess/RajLab Dropbox/Jess Li/Shared_JessL/paper/extractedData/dexmemfish/rep4",
                      "/Users/jess/RajLab Dropbox/Jess Li/Shared_JessL/paper/extractedData/dexmemfish/rep5",
                      "/Users/jess/RajLab Dropbox/Jess Li/Shared_JessL/paper/extractedData/dexmemfish/rep6"
)

outputdirectory <- "/Users/jess/RajLab Dropbox/Jess Li/Shared_JessL/paper/plots/dexmemfish"
...
plot <- ggplot() +
  ...
  ggsave(filename = paste0(outputdirectory, "/combineplot.pdf"), plot = plot, width = 12, height = 8, device = "pdf")
...
analytes <- c("cy3", "a594", "cy5")

# Function to analyze one analyte
analyze_analyte <- function(analyte_name, data) {
  # Fit robust model
  formula_str <- paste(analyte_name, "~ sample_identifier + (1 | dataset_identifier)")
  model <- rlmer(as.formula(formula_str), data = data)
  
  # Get emmeans results
  emmeans_result <- emmeans(model, pairwise ~ sample_identifier, adjust = "holm")
  
  # Return summary
  list(
    model = model,
    emmeans = emmeans_result,
    pairwise_summary = summary(emmeans_result$contrasts),
    normality_p = shapiro.test(data[[analyte_name]])$p.value
  )
}

# Analyze all analytes
all_results <- map(analytes, ~analyze_analyte(.x, mean_data)) %>%
  set_names(analytes)

# Print results for each analyte
iwalk(all_results, ~{
  cat("\n=== RESULTS FOR", toupper(.y), "===\n")
  cat("Normality test p-value:", format.pval(.x$normality_p), "\n")
  print(.x$pairwise_summary)
})

# Create summary table
final_summary <- map_dfr(names(all_results), ~{
  pairwise <- all_results[[.x]]$pairwise_summary
  data.frame(
    analyte = .x,
    comparison = pairwise$contrast,
    p_value = pairwise$p.value,
    significant = pairwise$p.value < 0.05
  )
})
...
# Fixed comprehensive results code
for(analyte in analytes) {
  cat("\n=== COMPREHENSIVE RESULTS FOR", toupper(analyte), "===\n")
  
  if(is.null(emmeans_results[[analyte]])) {
    cat("No results for", analyte, "\n")
    next
  }
  
  emmeans_result <- emmeans_results[[analyte]]
  
  tryCatch({
    # Get detailed statistics with consistent Holm adjustment
    contrast_summary <- summary(emmeans_result$contrasts)
    confidence_intervals <- confint(emmeans_result$contrasts, adjust = "holm")
    
    # Print comprehensive results
    cat("Test statistics (z-ratios):", paste(round(contrast_summary$z.ratio, 3), collapse = ", "), "\n")
    cat("Degrees of freedom:", paste(round(contrast_summary$df, 1), collapse = ", "), "\n")
    cat("Exact p-values (Holm-adjusted):", paste(format.pval(contrast_summary$p.value, digits = 4), collapse = ", "), "\n")
    
    cat("95% Confidence intervals (Holm-adjusted):\n")
    print(confidence_intervals)
    
    cat("Effect sizes (z-ratios):", paste(round(contrast_summary$z.ratio, 3), collapse = ", "), "\n")
    
    # Show significant comparisons
    sig_comparisons <- contrast_summary[contrast_summary$p.value < 0.05, ]
    if(nrow(sig_comparisons) > 0) {
      cat("Significant comparisons (p < 0.05):\n")
      for(i in 1:nrow(sig_comparisons)) {
        cat(sprintf("  %s: z = %.3f, p = %s\n", 
                    sig_comparisons$contrast[i], 
                    sig_comparisons$z.ratio[i], 
                    format.pval(sig_comparisons$p.value[i], digits = 4)))
      }
    } else {
      cat("No significant comparisons after Holm correction.\n")
    }
    
    # Summary statistics for the analyte
    cat("Summary for", toupper(analyte), ":\n")
    cat(sprintf("  Total comparisons: %d\n", nrow(contrast_summary)))
    cat(sprintf("  Significant comparisons: %d\n", sum(contrast_summary$p.value < 0.05)))
    cat(sprintf("  Range of estimates: %.6f to %.6f\n", 
                min(contrast_summary$estimate), max(contrast_summary$estimate)))
    
  }, error = function(e) {
    cat("Error processing", analyte, ":", e$message, "\n")
  })
  
  cat("\n")
}

# Report Shapiro-Wilk normality tests for raw data
iwalk(all_results, ~cat("Shapiro-Wilk test for raw", toupper(.y), "data: p =", format.pval(.x$normality_p), "\n"))
