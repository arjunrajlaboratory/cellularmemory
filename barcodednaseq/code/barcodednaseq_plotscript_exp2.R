...
input_directory <- "/Users/jessi/RajLab Dropbox/Jess Li/Shared_JessL/paper/extractedData/barcodednaseq/20230726/exp2/output"
output_directory <- "/Users/jessi/RajLab Dropbox/Jess Li/Shared_JessL/paper/plots/barcodednaseq/20230726/exp2"
...
#hi
a <- dplyr::filter(shavedStarcode, SampleNum == "JL_AT_5")
#lo
b <- dplyr::filter(shavedStarcode, SampleNum == "JL_AT_6")
#hi
c <- dplyr::filter(shavedStarcode, SampleNum == "JL_AT_7")
#lo->hi
d <- dplyr::filter(shavedStarcode, SampleNum == "JL_AT_8")
...
a <- dplyr::filter(a, UMI > 1.5)
b <- dplyr::filter(b, UMI > 1.5)
c <- dplyr::filter(c, UMI > 1.5)
d <- dplyr::filter(d, UMI > 1.5)
...
# Function to create and save Venn diagram as PDF
create_venn <- function(set_list, set_names, output_file) {
  venn <- venn.diagram(
    x = set_list,
    category.names = set_names,
    filename = NULL,  # We'll use NULL here and save manually as PDF later
    output = TRUE,
    lwd = 2,
    col = "black",
    alpha = 0.50,
...
combinations <- list(
  list(c("a", "b"), "ab_venn.pdf"),
  list(c("a", "c"), "ac_venn.pdf"),
  list(c("a", "d"), "ad_venn.pdf"),
  list(c("b", "c"), "bc_venn.pdf"),
  list(c("b", "d"), "bd_venn.pdf"),
  list(c("c", "d"), "cd_venn.pdf")
)
